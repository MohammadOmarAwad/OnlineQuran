...
export class PrayTimeComponent {
  ...
  //Run on Strat
  ngOnInit() {
    Utilities.getLocation().then(loc => { this.callAladhanApi(loc[0], loc[1]); });
    Utilities.getCityName().then(output => { this.CityName = output; });
  }
  ...
  callAladhanApi(Longitude: String, Latitude: String) {
    ...
    const year = Utilities.GrtHijriDateYear();
    const monthe = Utilities.GrtHijriDateMonth();
